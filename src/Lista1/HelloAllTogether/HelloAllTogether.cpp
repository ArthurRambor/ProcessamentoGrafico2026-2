...
int main()
{
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Ola tudo junto ! -- Arthur", nullptr, nullptr);
	if (!window)
	{
	...
	while (!glfwWindowShouldClose(window))
	{
		// Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
			{
				...
				char tmp[256];
				sprintf(tmp, "Ola tudo junto! -- Arthur\tFPS %.2lf", fps);
		...
		glDrawArrays(GL_LINE_LOOP, 0, 3); // T0: v0, v1, v2 → fecha o triângulo sozinho
		glDrawArrays(GL_LINE_LOOP, 3, 3); // T1: v3, v4, v5 → fecha o outro triângulo sozinho
        glUniform4f(colorLoc, 1.0f, 0.0f, 0.0f, 1.0f); // enviando cor para variável uniform inputColor
		glDrawArrays(GL_POINTS, 0, 6);  // apenas os vértices como pontos
